...
84 теста на встроенном раннере Node, без зависимостей. Весь набор проходит за 1.3 секунды.
Что закреплено:
...
| `test/site.test.mjs` | скрипт лендинга разбирается, нет вызовов удалённых функций, шаблон uCoz собран из `index.html`, в подвале нет личной почты |
| `test/crawl-live.test.mjs` | выводы, зависящие от сети, на настоящем локальном сервере: два адреса через перенаправление на один раздел не считаются дублем, настоящие дубли по-прежнему находятся, ответ без HTML-тела получает проверку кода ответа |
| `test/har.test.mjs` | HAR по спецификации: у каждого запроса своя метка времени, `time` равен сумме `timings`, негодный адрес даёт отказ, а не исключение |
