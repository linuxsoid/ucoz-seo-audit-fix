...
Набор файлов: три отчёта для человека (MD, HTML и машинный JSON), два для ИИ (русский и
английский) и пять выгрузок, которые обычно забирают из DevTools руками, то есть HAR со всеми
сетевыми запросами, лог консоли, официальный отчёт Lighthouse в HTML и в JSON и скриншот
страницы целиком. Архив собирается своим кодом на `node:zlib`, зависимостей для этого нет.
