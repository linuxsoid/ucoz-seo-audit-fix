 ...
 *
 * Проверка идёт в два запроса, и это разделение не техническое, а про человека. Первый,
 * /api/audit, обходит страницы обычными HTTP-запросами и отвечает за несколько секунд:
 * посетитель сразу видит результат, а не пустой экран. Второй, /api/deep, поднимает
 * настоящий Chrome, снимает Lighthouse и браузерную диагностику и занимает около минуты.
 *
 * Второй запрос дороже первого примерно в двадцать раз, поэтому у него свои ограничения:
 * лимит по IP строже втрое, браузерный слот один на весь сервис и очередь ограничена.
 * Без этого один посетитель занимал бы машину целиком.
 *
 * Результат живёт в памяти четверть часа: по ссылке его можно посмотреть страницей
 * (/api/view) и забрать одним архивом (/api/bundle).
 *
 * Запуск:
...
/**
 * Набор файлов результата.
 *
 * Один и тот же набор идёт и в архив, и на живую страницу отчёта: страница берёт из него
 * готовый HTML. Если бы наборы собирались отдельно, человек однажды увидел бы на странице
 * не то, что лежит в скачанном архиве.
 */
...
async function handleDeepAudit(req, res) {
  ...
  try {
    ...
    return sendJson(res, 200, {
      ...
      // Тексты отчётов в ответе больше не нужны: человек читает их живой страницей, а
      // забирает архивом. Восемьдесят килобайт готового markdown и HTML внутри JSON
      // никто не читал, а ответ они удваивали.
